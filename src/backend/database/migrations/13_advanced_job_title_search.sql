-- Migration: Advanced Job Title Search 
-- Description: Enhanced function for searching contacts by job titles and executive positions

-- Drop existing function to avoid conflicts
DROP FUNCTION IF EXISTS search_contacts_optimized(uuid, text, integer, integer);
DROP FUNCTION IF EXISTS search_contacts_optimized(uuid, text, integer, integer, text[]);

-- Create improved search_contacts_optimized function with optimized logic
...
COMMENT ON FUNCTION search_contacts_optimized IS 'Highly optimized contact search with intelligent job title filtering and executive search capabilities';
