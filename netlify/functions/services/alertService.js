...
class AlertService {
    ...
    async createAlert(userId, alertData) {
        try {
            ...
            if (alertData.contact_id && !(0, validation_1.isValidUUID)(alertData.contact_id)) {
                return {
                    success: false,
                    error: {
            ...
            if (alertData.event_id && !(0, validation_1.isValidUUID)(alertData.event_id)) {
                return {
                    success: false,
                    error: {
    ...
    async updateAlert(userId, alertId, updateData) {
        try {
            ...
            if (!alertId || !(0, validation_1.isValidUUID)(alertId)) {
                return {
                    success: false,
                    error: {
            ...
            // Validate contact_id if provided
            if (updateData.contact_id && !(0, validation_1.isValidUUID)(updateData.contact_id)) {
                return {
                    success: false,
                    error: {
                        code: 'INVALID_CONTACT_ID',
            ...
            // Validate event_id if provided
            if (updateData.event_id && !(0, validation_1.isValidUUID)(updateData.event_id)) {
                return {
                    success: false,
                    error: {
                        code: 'INVALID_EVENT_ID',
            ...
            const { data: existingAlert, error: checkError } = await supabaseAdmin_1.supabaseAdmin
                .from('alerts')
                .select('*')
                .eq('alert_id', alertId)
                .eq('user_id', userId)
     ...
     * Deletes an alert
     */
    async deleteAlert(userId, alertIdentifier) {
        try {
            if (!userId) {
                return {
                    success: false,
            ...
            if (alertIdentifier.type === 'id') {
                if (!(0, validation_1.isValidUUID)(alertIdentifier.alert_id)) {
                    return {
                        success: false,
                        error: {
                            code: 'INVALID_ALERT_ID',
                ...
                const { data: alertData, error: fetchError } = await supabaseAdmin_1.supabaseAdmin
                    .from('alerts')
                    .select('title')
                    .eq('alert_id', alertId)
                    .eq('user_id', userId)
     ...
     */
    async findAlerts(userId, options = {}) {
        try {
            if (!userId) {
                return {
                    success: false,
     ...
     */
    async getUpcomingAlerts(userId, daysAhead = 7, limit = 10) {
        try {
            if (!userId) {
                return {
                    success: false,
     ...
     */
    async snoozeAlert(userId, alertId, snoozeUntil) {
        try {
            if (!userId) {
                return {
                    success: false,
