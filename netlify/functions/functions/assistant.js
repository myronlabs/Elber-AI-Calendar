...
function updateConversationState(userId, responseId) {
            ...
            for (const toolCall of assistantMessage.tool_calls) {
                ...
                if (args.operation_type === 'calendar') {
                    ...
                    if (args.action === 'create' && args.entity_data) {
                        const startTime = args.entity_data.start_time;
                        if (startTime) {
                    ...
                    if (args.search_criteria) {
                        ...
                        if (args.search_criteria.start_date) {
                            const startDate = new Date(args.search_criteria.start_date);
                        ...
                        if (args.search_criteria.end_date) {
                            const endDate = new Date(args.search_criteria.end_date);
...
async function executeUniversalOperation(args, userId, event, logPrefix, _userTimezone) {
...
// Contact operations
async function executeContactOperation(action, entity_data, search_criteria, userId, logPrefix) {
    try {
        switch (action) {
            case 'create': {
                if (!entity_data) {
                ...
                const filteredData = Object.fromEntries(Object.entries(contactFields).filter(([_, value]) => value !== null && value !== undefined));
                ...
                try {
                    const { clearUserCache } = await Promise.resolve().then(() => __importStar(require('./contacts-search')));
            ...
            case 'list': {
                ...
                if (search_criteria) {
                    ...
                    if ('search_term' in search_criteria && search_criteria.search_term) {
                        const searchTerm = search_criteria.search_term;
                        ...
                        if (searchTerm.toLowerCase().includes('duplicate')) {
                            ...
                            if (searchTerm.includes(' ')) {
                                ...
                                if (parts.length === 2) {
                                    ...
                                    query = query.or(`and(first_name.ilike.%${first}%,last_name.ilike.%${last}%),` +
            ...
            case 'update': {
                ...
                if (search_criteria && 'contact_id' in search_criteria && search_criteria.contact_id) {
                    ...
                    const { data: contact, error: findError } = await supabaseAdmin_1.supabaseAdmin
                        .from('contacts')
                        .select('*')
...
// Calendar operations
async function executeCalendarOperation(action, entity_data, search_criteria, userId, event, logPrefix) {
    try {
        ...
        if (action === 'create' && entity_data) {
            ...
            if (search_criteria) {
                if (search_criteria.search_term) {
                    queryParams.append('search_term', search_criteria.search_term);
                }
                if (search_criteria.start_date) {
                    queryParams.append('start_date', search_criteria.start_date);
                }
                if (search_criteria.end_date) {
                    queryParams.append('end_date', search_criteria.end_date);
                }
            ...
            if (search_criteria && search_criteria.event_id) {
                eventId = search_criteria.event_id;
            }
            else if (entity_data.event_id) {
                eventId = entity_data.event_id;
            }
            if (!eventId) {
                return {
                    success: false,
            ...
            // Remove null/undefined fields
            const filteredPayload = Object.fromEntries(Object.entries(updatePayload).filter(([_, value]) => value !== null && value !== undefined));
            ...
            if (search_criteria && search_criteria.event_id) {
                ...
                const eventId = search_criteria.event_id;
                const response = await fetch(`${baseUrl}/.netlify/functions/calendar/${eventId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': authHeader
                ...
                if (searchResults.length > 1) {
                    ...
                    const eventList = searchResults.map((event) => `- ${event.title} (${new Date(event.start_time).toLocaleString()})`).join('\n');
                    return {
                        success: false,
...
// Alert operations
async function executeAlertOperation(action, entity_data, search_criteria, userId, logPrefix) {
    try {
        switch (action) {
            case 'create': {
                if (!entity_data) {
                ...
                // Remove null/undefined fields
                const filteredData = Object.fromEntries(Object.entries(alertFields).filter(([_, value]) => value !== null && value !== undefined));
                const { data, error } = await supabaseAdmin_1.supabaseAdmin
                    .from('alerts')
                    .insert({ ...filteredData, user_id: userId })
                    .select()
...
// Settings operations
async function executeSettingsOperation(action, entity_data, userId, event, logPrefix) {
    try {
...
async function executeDuplicateManagementOperation(action, _entity_data, search_criteria, userId, logPrefix) {
    ...
    try {
        switch (action) {
            ...
            case 'delete': {
                ...
                const { data: existingContact, error: checkError } = await supabaseAdmin_1.supabaseAdmin
                    .from('contacts')
                ...
                const { error, count } = await supabaseAdmin_1.supabaseAdmin
                    .from('contacts')
                    .delete()
                    .eq('contact_id', contactId)
                    .eq('user_id', userId);
