...
const handler = async (event, _context) => {
    ...
    const ipRateLimit = await rateLimiter_1.authRateLimiter.isAllowed(clientIP, 'passwordReset', rateLimiter_1.RATE_LIMITS.passwordReset);
    ...
    const emailRateLimit = await rateLimiter_1.authRateLimiter.isAllowed(`email:${emailIdentifier}`, 'passwordReset', rateLimiter_1.RATE_LIMITS.passwordReset);
    ...
    try {
        ...
        const { data: resetData, error: resetError } = await services_1.supabaseAdmin.auth.admin.generateLink({
            type: 'recovery',
            email: email,
            options: {
        ...
        if (resetData && resetData.properties && resetData.properties.action_link) {
            ...
            const resendResult = await (0, services_1.sendPasswordResetEmail)(email, resetLink);
            ...
            await rateLimiter_1.authRateLimiter.reset(clientIP, 'passwordReset');
            await rateLimiter_1.authRateLimiter.reset(`email:${emailIdentifier}`, 'passwordReset');
        }
        else {
