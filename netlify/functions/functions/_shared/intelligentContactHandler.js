...
exports.handleIntelligentContactOperation = handleIntelligentContactOperation;
const supabaseAdmin_1 = require('./services/supabaseAdmin');
const contactService_1 = require('./services/contactService');
const contacts_search_1 = require("../contacts-search");
/**
 * Detects if a search query is too vague and needs clarification
 */
function isVagueSearchQuery(userRequest, searchTerm) {
 ...
 */
function validateCRUDOperation(args) {
 ...
 */
async function handleIntelligentContactOperation(args, userId, context) {
 ...
 */
async function handleIntelligentSearch(args, userId, context) {
    const { logPrefix } = context;
    if (!args.search_term) {
        return {
            success: false,
 ...
 */
async function handleIntelligentCreate(args, userId, context) {
    const { logPrefix } = context;
    if (!args.contact_updates?.first_name || !args.contact_updates?.last_name) {
        return {
            success: false,
    ...
    const createData = { user_id: userId };
    ...
    const { data: newContact, error } = await supabaseAdmin_1.supabaseAdmin
        .from('contacts')
        .insert(createData)
        .select()
        .single();
    ...
    (0, contacts_search_1.clearUserCache)(userId);
 ...
 */
async function handleIntelligentUpdate(args, userId, context) {
    const { logPrefix } = context;
    ...
    for (const contact of contacts) {
        ...
        // Perform update
        const { data: supabaseResponseData, error: updateError } = await supabaseAdmin_1.supabaseAdmin
            .from('contacts')
 ...
 */
function detectPhoneFieldIntent(userRequest, contactUpdates) {
    const mapping = {};
    if (!contactUpdates)
        return mapping;
 ...
 */
async function handleIntelligentDelete(args, userId, context) {
    const { logPrefix } = context;
    if (!args.search_term) {
        return {
            success: false,
    ...
    const deleteResult = await contactService_1.contactService.deleteContact(userId, {
        type: 'name',
 ...
 */
async function handleIntelligentMerge(args, userId, context) {
    const { logPrefix } = context;
    if (!args.search_term) {
        return {
            success: false,
    ...
    const duplicateIds = contacts
        .filter((c) => c.contact_id !== primaryContact.contact_id)
        .map((c) => c.contact_id);
...
function calculateCompletenessScore(contact) {
    let score = 0;
    const importantFields = ['first_name', 'last_name', 'email', 'phone', 'mobile_phone', 'company', 'job_title'];
...
//# sourceMappingURL=intelligentContactHandler.js.map