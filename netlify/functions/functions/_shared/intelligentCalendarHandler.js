...
exports.handleIntelligentCalendarOperation = handleIntelligentCalendarOperation;
const date_fns_1 = require("date-fns");
/**
 * Validates calendar operation arguments for completeness and correctness
 */
function validateCalendarOperation(args) {
 ...
 */
async function handleIntelligentCalendarOperation(args, userId, context) {
 ...
 */
async function handleIntelligentCalendarSearch(args, userId, context) {
    const { logPrefix } = context;
 ...
 */
async function handleIntelligentCalendarCreate(args, userId, context) {
    const { logPrefix } = context;
    if (!args.event_data?.title || !args.event_data?.start_time || !args.event_data?.end_time) {
        return {
            success: false,
    ...
    let endTime;
    try {
        startTime = (0, date_fns_1.parseISO)(args.event_data.start_time);
        endTime = (0, date_fns_1.parseISO)(args.event_data.end_time);
 ...
 */
async function handleIntelligentCalendarUpdate(args, userId, context) {
    const { logPrefix } = context;
    if (!args.search_criteria) {
        return {
            success: false,
 ...
 */
async function handleIntelligentCalendarDelete(args, userId, context) {
    if (!args.search_criteria) {
        return {
            success: false,
            operation: "delete",
...
//# sourceMappingURL=intelligentCalendarHandler.js.map