...
exports.getSystemPromptFromEnvironmentOrDefault = getSystemPromptFromEnvironmentOrDefault;
exports.enrichPrompt = enrichPrompt;
exports.callOpenAI = callOpenAI;
exports.makeOpenAIRequest = makeOpenAIRequest;
exports.extractIntentionFromResponse = extractIntentionFromResponse;
/**
 * Get system prompt from environment variable or use default
 */
function getSystemPromptFromEnvironmentOrDefault(envVarName, defaultPrompt) {
    return process.env[envVarName] || defaultPrompt;
}
/**
 * Enrich a prompt with additional context
 */
function enrichPrompt(prompt, context) {
 ...
 */
async function callOpenAI(messages, systemPrompt, apiKey, model = 'gpt-4.1-mini') {
    try {
 ...
 */
async function makeOpenAIRequest(messages, systemPrompt, apiKey, model = 'gpt-4.1-mini') {
    return callOpenAI(messages, systemPrompt, apiKey, model);
}
/**
 * Extract intention from OpenAI's response
 */
function extractIntentionFromResponse(userMessage, _response) {
...
function determineAction(message) {
    // Create
    if (message.includes('create alert') ||
    ...
    // Update
    if (message.includes('update alert') ||
    ...
    // Delete
    if (message.includes('delete alert') ||
        message.includes('remove alert') ||
        message.includes('cancel alert')) {
        return 'delete_alert';
    }
    // Get
    if (message.includes('get alert') ||
    ...
    // Dismiss
    if (message.includes('dismiss alert') ||
    ...
    // Snooze
    if (message.includes('snooze alert') ||
    ...
    // List/Search
    if (message.includes('list alerts') ||
