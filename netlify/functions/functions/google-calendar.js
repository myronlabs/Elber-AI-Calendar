...
const handler = async (event, _context) => {
    ...
    try {
        ...
        const { data: tokenData, error: fetchError } = await services_1.supabaseAdmin
            ...
            case 'disconnect': {
                if (event.httpMethod === 'POST') {
                    ...
                    try {
                        ...
                        const { error: deleteError } = await services_1.supabaseAdmin
                            .from('oauth_connections')
                            .delete()
                            .eq('user_id', userId)
                            .eq('provider', 'google');
