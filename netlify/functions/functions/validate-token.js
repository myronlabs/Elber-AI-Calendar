...
function createErrorResponse(message, statusCode, errorType) {
    return {
        statusCode,
        headers: {
...
async function handler(event, _context) {
    ...
    try {
        ...
        if (profileError || !profileData) {
            console.error(`[validate-token] Request ${requestId}: Failed to fetch verification status for user ${data.user.id}:`, profileError || 'No profile found');
            clearTimeout(timeoutId);
        ...
        clearTimeout(timeoutId);
        return createErrorResponse('Error validating token', 500, 'internal_error');
    }
}
