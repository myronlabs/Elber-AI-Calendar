...
const handler = async (event, _context) => {
    ...
    try {
        ...
        if (isAutoVerification) {
            ...
            const { data: userExists, error: userError } = await services_1.supabaseAdmin
                .from('profiles')
                .select('id')
                .eq('id', userId)
                .single();
            ...
            const result = await services_1.supabaseAdmin
        ...
        const { error: profileUpdateError } = await services_1.supabaseAdmin
            .from('profiles')
        ...
        const { error: updateUserError } = await services_1.supabaseAdmin.auth.admin.updateUserById(userId, { email_confirm: true } // This tells Supabase to set email_confirmed_at
        );
        if (updateUserError) {
        ...
        if (verificationData) {
            const { error: deleteCodeError } = await services_1.supabaseAdmin
                .from('verification_codes')
                .delete()
                .eq('user_id', userId)
                .eq('code', token);
