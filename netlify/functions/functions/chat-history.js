 ...
 */
async function getAuthenticatedUser(event, requestId) {
    ...
    try {
        ...
        return {
            user: null,
            response: createErrorResponse('Authentication error during token verification', 401, 'auth_verification_error', {
 ...
 */
async function getChatHistory(userId, requestId) {
    try {
        ...
        if (error) {
            ...
            return createErrorResponse('Failed to retrieve chat history', 500, 'db_error', error.message);
        }
        // Extract message objects from the database results
        const messages = data?.map((item) => item.message_object) || [];
        ...
        if (error instanceof Error && error.message === 'Database query timeout') {
            return createErrorResponse('Database query timed out', 504, 'db_timeout');
        }
        return createErrorResponse('Error retrieving chat history', 500, 'internal_error', error instanceof Error ? error.message : String(error));
    }
}
/**
 * Save a new message to the database
 */
async function saveMessage(userId, message, requestId) {
    try {
        ...
        if (error) {
            ...
            return createErrorResponse('Failed to save message', 500, 'db_error', error.message);
        }
        console.log(`[chat-history] Request ${requestId}: Message saved successfully with ID ${data?.[0]?.id}`);
        return createSuccessResponse({ id: data?.[0]?.id, created_at: data?.[0]?.created_at }, { message: 'Message saved successfully' }, 201 // Created
        );
    }
    catch (error) {
        ...
        if (error instanceof Error && error.message === 'Database operation timeout') {
            return createErrorResponse('Database operation timed out', 504, 'db_timeout');
        }
        if (error instanceof SyntaxError) {
            return createErrorResponse('Invalid JSON format', 400, 'syntax_error', error.message);
        }
        return createErrorResponse('Error saving message', 500, 'internal_error', error instanceof Error ? error.message : String(error));
    }
}
/**
 * Delete all chat history for a user
 */
async function clearChatHistory(userId, requestId) {
    try {
        ...
        if (error) {
            ...
            return createErrorResponse('Failed to clear chat history', 500, 'db_error', error.message);
        }
        console.log(`[chat-history] Request ${requestId}: Successfully deleted ${count || 0} messages for user ${userId}`);
        return createSuccessResponse({ deletedCount: count || 0 }, { message: 'Chat history cleared successfully' });
    }
    catch (error) {
        ...
        return createErrorResponse('Error clearing chat history', 500, 'internal_error', error instanceof Error ? error.message : String(error));
    }
}
/**
...
const handler = async (event, _context) => {
    ...
    try {
        ...
        try {
            ...
            switch (event.httpMethod) {
                ...
                case 'POST': {
                    ...
                    try {
                        ...
                        clearTimeout(timeoutId);
                        return createErrorResponse('Invalid JSON in request body', 400, 'invalid_json', parseError instanceof Error ? parseError.message : String(parseError));
                    }
                }
                case 'DELETE': {
                    // Clear chat history
            ...
            clearTimeout(timeoutId);
            return createErrorResponse('Error processing request', 500, 'processing_error', innerError instanceof Error ? { message: innerError.message, stack: innerError.stack } : String(innerError));
        }
    }
    catch (error) {
        ...
        clearTimeout(timeoutId);
        return createErrorResponse('An unexpected error occurred', 500, 'internal_error', error instanceof Error ? { message: error.message, stack: error.stack } : String(error));
    }
    finally {
