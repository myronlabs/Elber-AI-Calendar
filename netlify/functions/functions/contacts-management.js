...
(function (ErrorCode) {
    ...
    ErrorCode["DUPLICATE_FOUND"] = "DUPLICATE_FOUND";
...
const setOperationContext = async (supabase, context) => {
    try {
...
const retryOperation = async (operation, maxRetries = 3, baseDelay = 100, shouldRetry) => {
    let lastError = null;
...
const checkRateLimits = async (userId, operation, size) => {
    try {
        // Input validation
        ...
        const { data, error } = await retryOperation(async () => await services_1.supabaseAdmin.rpc('rate_limit_contact_operations', {
            p_user_id: userId,
...
const batchUpdateContacts = async (userId, contactIds, updates, operationContext = 'batch_update_api') => {
    ...
    try {
        const { data: ownershipData, error: ownershipError } = await services_1.supabaseAdmin.rpc('check_contact_ownership', { p_user_id: userId, p_contact_ids: contactIds });
        if (ownershipError) {
        ...
        const nonOwnedContacts = ownershipData.filter((item) => !item.exists_for_user);
    ...
    try {
        ...
        await setOperationContext(services_1.supabaseAdmin, operationContext);
        // Execute the batch update with retry for transient errors
        const { data, error } = await retryOperation(async () => await services_1.supabaseAdmin.rpc('batch_update_contacts', {
            p_user_id: userId,
...
const batchDeleteContacts = async (userId, contactIds, operationContext = 'batch_delete_api') => {
    // Validate batch size
    if (contactIds.length > MAX_BATCH_SIZE) {
        return {
            success: false,
    ...
    try {
        ...
        const { data, error } = await services_1.supabaseAdmin.rpc('batch_delete_contacts', {
            p_user_id: userId,
            p_contact_ids: contactIds,
            p_operation_context: operationContext
        });
...
// Batch tag contacts
const batchTagContacts = async (userId, contactIds, tagNames, operationContext = 'batch_tag_api') => {
    // Validate batch size
    if (contactIds.length > MAX_BATCH_SIZE) {
        return {
            success: false,
    ...
    try {
        ...
        const { data, error } = await services_1.supabaseAdmin.rpc('batch_tag_contacts', {
            p_user_id: userId,
            p_contact_ids: contactIds,
...
const findDuplicateContacts = async (userId, params) => {
    try {
        // Use the find duplicates function to get potential duplicates
        const { data, error } = await services_1.supabaseAdmin.rpc('find_duplicate_contacts', {
            p_user_id: userId,
        ...
        if (data && data.length > 0) {
            ...
            const contactIds = data.map((dup) => dup.contact_id);
            // Fetch the full contact details for these IDs
            const { data: contactsData, error: contactsError } = await services_1.supabaseAdmin
                .from('contacts')
            ...
            const enrichedData = data.map((duplicate) => {
...
const getContactAuditHistory = async (userId, contactId, limit = 50, offset = 0) => {
    try {
        // First verify that the contact belongs to the user
        const { data: contactData, error: contactError } = await services_1.supabaseAdmin
            .from('contacts')
            .select('contact_id')
            .eq('user_id', userId)
...
const getOperationsHistory = async (userId, limit = 50, offset = 0) => {
    try {
...
const getImportQueueStatus = async (userId) => {
    try {
        ...
        // Get import history
        const { data: historyData, error: historyError } = await services_1.supabaseAdmin
            .from('import_history')
            .select('*')
            .eq('user_id', userId)
            .order('created_at', { ascending: false })
    ...
    if (httpMethod === 'POST') {
        ...
        switch (operation) {
            ...
            case 'find-duplicates':
                response = await findDuplicateContacts(userId, {
            ...
            case 'batch-update':
                ...
                response = await batchUpdateContacts(userId, requestBody.contactIds, requestBody.updates, requestBody.operationContext);
                break;
            case 'batch-delete':
                if (!requestBody.contactIds || !Array.isArray(requestBody.contactIds) || requestBody.contactIds.length === 0) {
                    return {
                ...
                response = await batchDeleteContacts(userId, requestBody.contactIds, requestBody.operationContext);
                break;
            case 'batch-tag':
                if (!requestBody.contactIds || !Array.isArray(requestBody.contactIds) || requestBody.contactIds.length === 0) {
                    return {
                ...
                response = await batchTagContacts(userId, requestBody.contactIds, requestBody.tagNames, requestBody.operationContext);
                break;
            default:
                return {
                    statusCode: 404,
...
//# sourceMappingURL=contacts-management.js.map